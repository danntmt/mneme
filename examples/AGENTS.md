...
2. **Open a session** — Call `mneme_open_session` with `workspaceId` set to a stable
   project identifier (e.g., the repository name or root directory name). The returned object
   contains a `sessionId` — **retain it** for all subsequent calls in this conversation.
...
|---------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------|
| `workspaceId` | A stable project identifier. Use the repository or directory name (e.g., `"my-app"`). Must be consistent across sessions so entries accumulate correctly. |
...
| `topic`       | A grouping label for entries (e.g., `"architecture"`, `"testing"`, `"deployment"`, `"api"`).                                                              |
| `confidence`  | A value from 0.0 to 1.0 indicating certainty. Default is 0.8. Use lower values for uncertain or inferred information.                                     |
